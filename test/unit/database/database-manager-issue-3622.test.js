  ...
  describe('意味のない文字列パターンの処理', () => {
    ...
    it('意味のない文字列が適切に処理される', () => {
      const testCases = [
        ...
        { error: '\n', commandIndex: 4, expected: 'Redis command 4 failed: Invalid response (empty/dash). This may indicate: 1. Redis connection timeout or instability, 2. Redis server memory pressure or resource exhaustion, 3. Network connectivity issues between application and Redis, 4. Redis client library response parsing issues, 5. Redis transaction queue overflow or corruption. Recovery actions: Connection retry with exponential backoff, Circuit breaker activation, Transaction re-validation' },
      ];
  ...
  describe('2PC統合テスト - 実際のエラーパターン', () => {
    ...
    it('Issue #3622で報告されたエラーパターンが適切に処理される', () => {
      ...
      expect(errorDetails).toBe(
        'コマンド0: Redis command 0 failed: Invalid response (empty/dash). This may indicate: 1. Redis connection timeout or instability, 2. Redis server memory pressure or resource exhaustion, 3. Network connectivity issues between application and Redis, 4. Redis client library response parsing issues, 5. Redis transaction queue overflow or corruption. Recovery actions: Connection retry with exponential backoff, Circuit breaker activation, Transaction re-validation, ' +
