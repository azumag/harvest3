# 暗号通貨取引ボット (scalping-bot)

このプロジェクトは、複数の取引戦略を実装した暗号通貨取引ボットです。ccxtライブラリを使用して、複数の取引所（bitbank、bitflyer）に接続し、自動取引を行います。SQLiteデータベースを使用して取引記録や状態を永続化し、Web UIを通じて状態監視や操作が可能です。
...
このボットは取引記録や状態の永続化に **SQLite** を使用しています。データベースファイルはプロジェクトルートの `data/` ディレクトリ内に作成されます (`trade_records.db` など)。

### データベースマイグレーション

データベーススキーマの変更などに対応するため、マイグレーションスクリプトが用意されています。古いデータ形式から新しい形式へ移行する場合などに使用します。

```bash
# マイグレーションを実行 (例: JSON形式からSQLiteへ)
npm run migrate-to-sqlite
```

**注意:** Dockerを使用している場合、`data/` ディレクトリがボリュームとしてマウントされているため、コンテナを再作成してもデータは保持されます。
...
-   **ポジション (`/positions.html`)**: 現在のポジション状況（通貨ペア、数量、価格など）を一覧で確認できます。
-   **履歴 (`/history.html`)**: 過去の取引履歴を詳細に確認できます。フィルタリングやソートも可能です。
-   **分析 (`/analysis.html`)**: 損益グラフや取引統計など、ボットのパフォーマンスを分析するための情報を提供します。
-   **イベント (`/events`)**: データベースイベントのリアルタイム表示 (Server-Sent Events)。

### 使い方

1.  ボット (例: `bot`, `hft`, `mm` のいずれか) と Web UI サーバー (`web-ui` または `npm run start-web`) を起動します。
    ```bash
    # Dockerの場合
    docker compose up -d bot web-ui
    # または 通常実行の場合 (別々のターミナルで)
    # npm start
    # npm run start-web
    ```
2.  Webブラウザで `http://localhost:3000` にアクセスします。
