  ...
  _crossValidation(performanceResults) {
    ...
    try {
      ...
      for (let i = 0; i < folds; i++) {
        ...
        if (testData.length === 0 || trainData.length === 0) {
          continue;
        }
        ...
        if (trainReturns.length === 0 || testReturns.length === 0) {
          continue;
        }
        ...
        if (!isFinite(trainMean) || !isFinite(testMean) || isNaN(trainMean) || isNaN(testMean)) {
          continue;
        }
