...
async function processDiscrepancies(discrepancies, exchangeId, diagnosticInfo) {
  ...
  if (veryHighExternalTradeDiscrepancies.length > 0 && 
    ...
    // 90%以上の外部取引の比率が閾値以上の場合、または1つでも存在する場合、全体をINFOレベルとして扱う
    if (veryHighRatio >= LOG_LEVEL_CONFIG.VERY_HIGH_EXTERNAL_RATIO_THRESHOLD || veryHighExternalTradeDiscrepancies.length >= 1) {
      logLevel = 'info';
