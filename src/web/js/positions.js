      ...
      <div class="position-card ${cardClass} mb-3" id="position-card-${position.exchange}-${position.symbol.replace('/', '-')}-${position.strategy}">
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <h6 class="card-subtitle text-muted">${position.exchange} - ${position.symbol} - ${position.strategy}</h6>
            <div class="d-flex align-items-center flex-wrap justify-content-end">
              <span class="badge ${position.unrealizedPnL > 0 ? 'bg-success' : position.unrealizedPnL < 0 ? 'bg-danger' : 'bg-secondary'} me-1 mb-1">
                未実現損益: ${position.unrealizedPnL !== null && position.unrealizedPnL !== undefined ? position.unrealizedPnL.toLocaleString() : '0'} 円
              </span>
              <span class="badge bg-info text-dark me-1 mb-1">
                ${position.unrealizedPnLPercent !== null && position.unrealizedPnLPercent !== undefined ? position.unrealizedPnLPercent.toFixed(2) : '0'}%
              </span>
            </div>
          </div>
          <div class="row">
            <div class="col-6">
              <p class="mb-1"><strong>ポジション:</strong> ${position.side || 'N/A'}</p>
              <p class="mb-1"><strong>数量:</strong> ${position.amount !== null && position.amount !== undefined ? position.amount.toLocaleString() : '0'}</p>
              <p class="mb-1"><strong>エントリー価格:</strong> ${position.entryPrice !== null && position.entryPrice !== undefined ? position.entryPrice.toLocaleString() : '0'} 円</p>
            </div>
            <div class="col-6">
              <p class="mb-1"><strong>現在価格:</strong> ${position.currentPrice !== null && position.currentPrice !== undefined ? position.currentPrice.toLocaleString() : '0'} 円</p>
              <p class="mb-1"><strong>作成日時:</strong> ${position.createdAt ? new Date(position.createdAt).toLocaleString('ja-JP') : 'N/A'}</p>
              <p class="mb-1"><strong>経過時間:</strong> ${position.elapsedHours !== null && position.elapsedHours !== undefined ? position.elapsedHours.toFixed(1) : '0'} 時間</p>
            </div>
          </div>
        </div>
      </div>
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    // ダッシュボードページの場合、タブ切り替え時に初期化
    if (window.location.pathname.includes('index.html') || window.location.pathname === '/' || window.location.pathname === '') {
      const positionsDetailTab = document.getElementById('positions-detail-tab');
      if (positionsDetailTab) {
        let initialized = false;
        positionsDetailTab.addEventListener('shown.bs.tab', function() {
          if (!initialized) {
            initPositions();
            initialized = true;
          }
      ...
      // 独立したpositions.htmlページの場合は即座に初期化
      initPositions();
    }
  });
} else {
