  ...
  $('#filter-parameter-set').on('select2:select', async function (e) {
    ...
    if (selectedValue) {
      ...
      if (filterExchangeSelect) {
        filterExchangeSelect.value = exchange;
      }
      // 銘柄リストを更新して値を設定
      ...
      if (filterStrategySelect) {
        filterStrategySelect.value = strategy;
      }
 ...
 */
  function displayAnalysisParameterTable(params, container) {
    ...
    return `
                        <tr>
                            <td>${key}</td>
                            <td>${createSimpleInputField(key, value, type)}</td>
                            <td>${type}</td>
                        </tr>
                    `;
  }).join('')}
...
function addSignalsToChart(chart, signalData) {
  if (!chart || !signalData || signalData.length === 0) {
    return;
  }
  ...
  signalData.forEach((signal, idx) => {
    if (!signal.side || !signal.timestamp || !signal.price) {
      return;
    }
...
async function fetchParameterDetails(exchange, symbol, strategy) {
  try {
    ...
    if (filterTimeframeSelect) {
      filterTimeframeSelect.value = timeframe;
    }
    if (filterLimitSelect) {
      filterLimitSelect.value = limit;
    }
    ...
    if (filterTimeframeSelect) {
      filterTimeframeSelect.value = '1h';
    }
    if (filterLimitSelect) {
      filterLimitSelect.value = '100';
    }
  }
}
...
async function fetchParameterDetailsAndSetSelects(exchange, symbol, strategy) {
  try {
    ...
    // エラー時はセレクトボックスの値をデフォルトに戻すか、エラー表示を検討
    if (filterTimeframeSelect) {
      filterTimeframeSelect.value = '15m';
    }
    if (filterLimitSelect) {
      filterLimitSelect.value = '100';
    }
 ...
 */
// createInputField function moved to common-ui.js to eliminate duplication
...
function displayParameterTable(exchangeId, symbol, strategyKey, params, containerElement) {
  ...
  sortedParamNames.forEach(paramName => {
    ...
    tableHtml += CommonUI.createInputField(paramName, paramValue, paramKey); // createInputFieldを呼び出し
    tableHtml += '</td>';
    tableHtml += '</tr>';
  });
...
function handleParameterInputChange(e) {
  ...
  if (!input.matches('[data-param-key][data-param-name]')) {
    return;
  }
...
// テクニカル指標計算機能は共有ライブラリに移動
// Shared indicators from /src/common/indicators.js will be loaded via script tag