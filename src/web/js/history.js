...
function loadTradeHistory() {
  const tableBody = document.querySelector('#trades-history-table tbody');
    ...
    if (Array.isArray(updates)) {
      ...
      if (currentPage === 1) {
        updates.forEach(update => {
          if (update.type === 'trade_added' && matchesFilters(update.data)) {
        ...
        newItemsCount = updates.filter(update => 
          update.type === 'trade_added' && matchesFilters(update.data)
        ).length;
      }
