module.exports = {
  ...
  testTimeout: process.env.CI ? 120000 : 60000, // CI環境では2分に延長してタイムアウト問題を解決
  ...
  maxConcurrency: process.env.CI ? 2 : 5, // CI環境では並行実行を2に増加してパフォーマンス向上
  workerIdleMemoryLimit: process.env.CI ? '512MB' : '1GB', // CI環境でメモリ制限を512MBに設定（現代のテストスイートに対応）
  maxWorkers: process.env.CI ? 2 : '50%', // CI環境では2ワーカーでパフォーマンス向上とハング問題防止のバランス
