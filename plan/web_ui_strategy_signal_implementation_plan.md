...
2. **要件**:
   - ~~index.htmlに戦略ごとのシグナルサマリーを追加~~ (削除されました。詳細は `plan/strategy-signal-summary-removal.md` を参照)
   - シグナル履歴ページを新規追加
...
```mermaid
graph TD
    A[1. navbar.htmlにシグナル履歴メニュー追加] --> B[2. signal-history.html新規作成]
    B --> C[3. signal-history.js実装]
    C --> D[既存APIを使用してシグナルデータ取得]
```
