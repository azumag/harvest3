      ...
      for (const symbol of symbols) {
        ...
        for (const strategyKey of strategies) {
          ...
          try {
            ...
            // 新しい統計情報
            if (result.partialFills) totalPartialFills += result.partialFills;
            if (result.fullHashesAdded) totalFullHashesAdded += result.fullHashesAdded;
            if (result.contentHashesAdded) totalContentHashesAdded += result.contentHashesAdded;
            if (result.tradeIdsFixed) totalTradeIdsFixed += result.tradeIdsFixed;
          } catch (err) {
