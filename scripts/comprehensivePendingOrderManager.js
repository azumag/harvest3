...
class ComprehensivePendingOrderManager {
    ...
    calculateUrgencyLevel(stats, violations, health) {
        ...
        // レベル判定
        let level;
        if (score >= 70) level = '🚨 緊急';
        else if (score >= 40) level = '⚠️ 高';
    ...
    async preOrderValidation(symbol, side, price, amount) {
        ...
        try {
            ...
            if (!result.overall) {
                if (!validation.valid) console.log(`  バリデーション: ${validation.summary}`);
                if (!limitCheck.allowed) console.log(`  上限チェック: ${limitCheck.reason}`);
