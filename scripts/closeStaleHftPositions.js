 ...
 */
async function main() {
  console.log('1日取引がないHFTポジション強制決済スクリプトを開始します');
    ...
    for (const exchange of exchanges) {
      ...
      for (const summary of summaries) {
        if (summary.strategyKey === 'HFT' && summary.netPosition !== 0) {
          const updatedAt = moment(summary.updatedAt);
          if (updatedAt.isBefore(oneDayAgo)) {
            console.log(`  HFT戦略 ${summary.symbol} に1日以上取引がありません。ポジション: ${summary.netPosition}`);
