...
// 処理の統計情報
const processedExchanges = 0;
const processedSymbols = 0;
 ...
 */
async function mergeOutside(exchangeId, symbol) {
  console.log(`[${exchangeId}][${symbol}] OUTSIDE→マージを実行中`);

  // OUTSIDEサマリーキーとHFTサマリーキー
  const outsideKey = `trade:filledSummary:${exchangeId}:${symbol}:OUTSIDE`;
  const hftKey = `trade:filledSummary:${exchangeId}:${symbol}:INTER_EXCHANGE_ARBITRAGE`;
...
async function main() {
  try {
    ...
    try {
      // マージを実行
      const result = await mergeOutside('bitbank', 'XLM/JPY');
