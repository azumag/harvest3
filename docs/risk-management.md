...
3. **ポジション追跡**: ポジション情報はRedisに永続化されます
4. **Redis接続**: Redis接続エラー時は自動的にメモリストレージにフォールバックします
...
- ポジション情報はRedisに保存され、システム再起動後も保持されます
- 損益データも90日間Redisに保存されます
- 複数のボットプロセス間でポジション情報を共有できます

### フォールバック機能

Redis接続エラー時の安全機能：

- ポジション管理は自動的にメモリストレージにフォールバック
- 損益追跡も同様にフォールバック
- エラーログで状況を確認可能
...
```
position:{exchangeId}:{symbol}:{strategyKey}:{orderId}
pnl:{exchangeId}:{strategyKey}:{date}
```

## 今後の改善予定

- [x] ポジションストレージのRedis移行
- [ ] より詳細なリスクレポート機能
- [ ] 戦略ごとのリスク統計
- [ ] リスク管理ダッシュボード
- [ ] より効率的な全ポジション数チェック機能